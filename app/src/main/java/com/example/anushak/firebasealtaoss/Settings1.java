...
public class Settings1 extends AppCompatPreferenceActivity {
    ...
    public static class MainPreferenceFragment extends PreferenceFragment {
        ...
        public void onCreate(final Bundle savedInstanceState) {
            ...
            final SwitchPreference onOffFinger = (SwitchPreference) findPreference(this.getResources().getString(R.string.key_fingerprint));

            onOffFinger.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
                @Override
                public boolean onPreferenceChange(Preference preference, Object newValue) {
                    if (onOffFinger.isChecked()){
                        Toast.makeText(getActivity(), "Finger Print turned Off", Toast.LENGTH_SHORT).show();

                        onOffFinger.setChecked(false);

                        SharedPreferences fingerPrint = preference.getContext().getSharedPreferences("FingerPrint", MODE_PRIVATE);
                        SharedPreferences.Editor edit = fingerPrint.edit();
                        edit.clear();
                        edit.putBoolean("fingeraccess", false);
                        edit.commit();
                    }
                    else {
                        Toast.makeText(getActivity(), "Finger Print turned On", Toast.LENGTH_SHORT).show();
                        onOffFinger.setChecked(true);

                        SharedPreferences fingerprint = preference.getContext().getSharedPreferences("FingerPrint", MODE_PRIVATE);
                        SharedPreferences.Editor edit = fingerprint.edit();
                        edit.clear();
                        edit.putBoolean("fingeraccess", true);
                        edit.commit();
                    }
